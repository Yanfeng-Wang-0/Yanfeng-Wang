...
gome2_base_dir = Path("../Data/GOME2/final") # GOME-2 SIF 数据目录（按年份分文件夹）
...
# ==== 读取 GOME-2 SIF 数据 ====
results = []
for year_folder in sorted(gome2_base_dir.glob("*")):
    ...
    for month in range(1, 13):
        file_path = year_folder / f"GOME2B_SIF_{year}_{month:02d}_land.nc"
        if file_path.exists():
            ds = xr.open_dataset(file_path)
            var_name = list(ds.data_vars)[0]  # 自动获取变量名
            sif_mean = ds[var_name].mean().item() * 1000
...
# ==== 保存 CSV ====
output_csv = "../Data/ENSO/ONI/GOME2_SIF.csv"
results_df.to_csv(output_csv, index=False)
print(f"匹配结果已保存到 {output_csv}")

# ==== 绘图 ====
plt.figure(figsize=(12, 6))
plt.plot(results_df["Date"], results_df["SIF_mean"], label='GOME-2 SIF mean', color='blue')
...
plt.xlabel("Time")
plt.ylabel("Value")
plt.title("GOME-2 SIF vs ONI (2014-2020)")
plt.tight_layout()
plt.savefig("../Data/ENSO/ONI/GOME2_SIF_timeseries.png", dpi=300)
plt.show()
